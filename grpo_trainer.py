...
import logging
from medical_agent_core import DifficultyLevel, MedicalQuery, SpecialistResponse
...
class GRPOTrainer:
    ...
    def compute_advantages(self, rewards: List[float]) -> List[float]:
        ...
        std_reward = np.std(rewards_array) + 1e-8  # Add small epsilon to avoid division by zero
...
class CurriculumMARLTrainer:
    ...
    def train_curriculum_stage(self, stage_data: List, difficulty: DifficultyLevel) -> Dict[str, float]:
        ...
        return {
            ...
            'avg_loss': np.mean([m['loss'] for m in stage_metrics]),
            'avg_reward': np.mean([m['avg_reward'] for m in stage_metrics])
        }
...
if __name__ == "__main__":
    ...
    from medical_agent_core import RewardCalculator
