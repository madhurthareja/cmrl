...
async def medical_chat():
    ...
    try:
        ...
        if medical_agent is not None:
            # 1. Ask MedGemma (Attending) to classify intent
            clinical_intent = await medgemma_client.classify_clinical_intent(user_message)
            logger.info(f"MedGemma Triage: {clinical_intent}")
            
            if clinical_intent == "CHEST_PAIN":
                 logger.info("Activitating Symbolic Chest Pain Protocol (Llama Backend)")
                 # 2. Run the rigorous Symbolic Llama Workflow
                 symbolic_result = await medical_agent.process_symbolic_query(user_message, context=history_context)
                 
                 # 3. Feed the structured trace back to MedGemma for the final "Bedside Manner" response
                 protocol_trace = symbolic_result.reasoning
                 
                 synthesis_prompt = f"""
                 You are the Attending Physician. Your resident (Symbolic Engine) has run the Chest Pain Protocol.
                 
                 Resident's Findings & Trace:
                 {protocol_trace}
                 
                 Resident's Suggested Plan:
                 {symbolic_result.answer}
                 
                 Task: Explain these findings and the plan to the user clearly and professionally. 
                 Cite the protocol steps completed.
                 """
                 
                 medgemma_response = await medgemma_client.generate_text_response_async(synthesis_prompt)
                 answer_text = medgemma_response.get('answer', '')

                 # Prepare response metadata
                 assistant_metadata = {
                    'domain': symbolic_result.domain.value,
                    'difficulty_level': symbolic_result.difficulty_level.value,
                    'confidence': symbolic_result.confidence,
                    # We show the rigorous trace in the reasoning metadata
                    'reasoning': f"[Protocol Active: {clinical_intent}]\n\n{protocol_trace}",
                    'specialists_consulted': [s.specialist_type for s in symbolic_result.specialist_consultations],
                    'retrieved_context': [str(d) for d in symbolic_result.retrieved_context],
                    'model': f"{medgemma_client.config.model_name} (Attending) + Llama-Symbolic (Resident)",
                    'usage': medgemma_response.get('usage', {}),
                    'symbolic': True
                }

            else:
                # Standard RAG Path for General Queries
                try:
